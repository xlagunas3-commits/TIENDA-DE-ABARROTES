...
<div class="max-w-2xl mx-auto">
    <h3 class="text-2xl font-semibold text-gray-800 mb-6">Nuevo Artículo de Pedido</h3>

    <form action="{{ route('order-items.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow">
        @csrf
        ...
        <div class="mb-4">
            ...
            <select id="order_id" name="order_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('order_id') border-red-500 @enderror" required>
                ...
                @foreach($orders as $order)
                    <option value="{{ $order->id }}" @selected(old('order_id') == $order->id)>Pedido #{{ $order->id }} - Usuario: {{ $order->user->name ?? 'N/A' }}</option>
                @endforeach
            </select>
            @error('order_id')
        ...
        <div class="mb-4">
            ...
            <select id="product_id" name="product_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('product_id') border-red-500 @enderror" required>
                ...
                @foreach($products as $product)
                    <option value="{{ $product->id }}" @selected(old('product_id') == $product->id)>{{ $product->name }} - ${{ number_format($product->price, 2) }}</option>
                @endforeach
            </select>
            @error('product_id')
                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
        ...
        <div class="mb-4">
            ...
            <input type="number" id="quantity" name="quantity" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('quantity') border-red-500 @enderror" value="{{ old('quantity') }}" placeholder="1" min="1" required>
            @error('quantity')
                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
            @enderror
        </div>
