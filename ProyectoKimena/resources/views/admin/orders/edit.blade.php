...
<div class="max-w-2xl mx-auto">
    <h3 class="text-2xl font-semibold text-gray-800 mb-6">Editar Pedido</h3>

    <form action="{{ route('orders.update', $order) }}" method="POST" class="bg-white p-6 rounded-lg shadow">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select id="user_id" name="user_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('user_id') border-red-500 @enderror" required>
                ...
                @foreach($users as $user)
                    <option value="{{ $user->id }}" @selected(old('user_id', $order->user_id) == $user->id)>{{ $user->name }} ({{ $user->email }})</option>
                @endforeach
            </select>
            @error('user_id')
        ...
        <div class="mb-4">
            ...
            <input type="number" id="total" name="total" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('total') border-red-500 @enderror" value="{{ old('total', $order->total) }}" placeholder="0.00" required>
            @error('total')
                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <select id="status" name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('status') border-red-500 @enderror" required>
                <option value="">-- Seleccionar --</option>
                <option value="pending" @selected(old('status', $order->status) == 'pending')>Pendiente</option>
                <option value="completed" @selected(old('status', $order->status) == 'completed')>Completado</option>
                <option value="cancelled" @selected(old('status', $order->status) == 'cancelled')>Cancelado</option>
            </select>
            @error('status')
                <span class="text-red-600 text-sm mt-1">{{ $message }}</span>
            @enderror
